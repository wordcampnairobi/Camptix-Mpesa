...
class CampTix_Plugin {
	...
	function form_attendee_info() {
		...
		if ( isset( $this->error_flags['no_receipt_email'] ) && !isset($_POST['msisdn']))
		...
		if ( 'checkout' == $_GET['tix_action'] && isset($_POST['msisdn']) )
		{
			//$this->error( __( 'We have not recieved your payment yet.', 'camptix' ) );
		}
	...
	function form_checkout() {
		...
		if(isset($_POST['newPost']))
		{
			...
			$paymentUrl = $siteUrl . $uri_parts[0];
			$msisdn = substr($_POST['msisdn'], -9);
			$_POST = unserialize(base64_decode($_POST['newPost']));
			$_POST['msisdn'] = $msisdn;
			$verify_msisdn = $wpdb->get_results( $wpdb->prepare( "SELECT SQL_CALC_FOUND_ROWS id FROM wp_transation WHERE SUBSTR(wp_transation.sender_phone, -9) = %s AND date(wp_transation.transacton_date)=curdate() AND wp_transation.status='UNVERIFIED' order by wp_transation.id desc limit 1;", "$msisdn") );
			$total = $wpdb->get_var( "SELECT FOUND_ROWS();" );
			if('0' == $total)
			{
				$_POST['tix_tickets_selected'] = array();
				$this->error( __( 'We have not recieved your payment yet <a href="'.$paymentUrl.'">try again</a>.', 'camptix' ) );
			}
			else
			{
				$id = $verify_msisdn[0]->id;
				$wpdb->query( $wpdb->prepare( "UPDATE  wp_transation set status='VERIFIED' WHERE id = %d AND SUBSTR(sender_phone, -9) = %s LIMIT 1;", $id, "$msisdn" ) );
				$this->error( __( 'You have successfully bought the ticket.', 'camptix' ) );
			}
		}
		...
		foreach( (array) $_POST['tix_attendee_info'] as $i => $attendee_info ) {
			...
			//if ( ! isset( $attendee_info['ticket_id'] ) || ! array_key_exists( $_POST['tix_tickets_selected'], $this->tickets_selected ) ) {
			if ( ! isset( $attendee_info['ticket_id'] ) || ! array_key_exists( $attendee_info['ticket_id'], $_POST['tix_tickets_selected']) ) {
	...
	function payment_result( $payment_token, $result, $data = array() ) {
		...
		foreach ( $attendees as $attendee ) {
			...
			update_post_meta( $attendee->ID, 'tix_transaction_id', $transaction_id ); //comment
			update_post_meta( $attendee->ID, 'tix_transaction_details', $transaction_details ); //comment
	...
	function email_tickets( $payment_token = false, $from_status = 'draft', $to_status = 'publish' ) {
		...
		$newId = $attendees[0]->ID - 1;
		$order = get_post_meta( $newId, 'tix_order', true );
		$receipt_content = '';
		foreach ( $order['items'] as $item ) {
